...
#If BUILD_SALES = 1 Then ' Sales, PC, eCRBInfoSync, Batch のビルド時有効
    ''' <summary>
    ''' メニューカテゴリ
    ''' </summary>
    ''' <remarks></remarks>
    Public Enum FooterMenuCategory
        ...
        ''' <remarks></remarks>
        ShowRoomStatus = 1200
        '2012/11/16 TCS 藤井 【A.STEP2】次世代e-CRB  新車タブレット横展開に向けた機能開発 START
        ''' <summary>
        ''' TCV設定
        ''' </summary>
        ''' <remarks></remarks>
        TCVSetting = 1300
        '2012/11/16 TCS 藤井 【A.STEP2】次世代e-CRB  新車タブレット横展開に向けた機能開発 END
        '2013/10/31 TCS 山田 ADD i-CROP再構築後の新車納車システムに追加したリンク対応 START
        ''' <summary>
        ''' 新車納車システムへのリンク(LunaScape起動)
        ''' </summary>
        ''' <remarks></remarks>
        LinkMenu = 1400
        ...
        '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 START
        ''' <summary>
        ''' 商品紹介ボタン
        ''' </summary>
        ''' <remarks></remarks>
        PP = 1800
        '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 END
...
#Region "定数"
        ...
        Private Const C_MSTPG_FOTTER_DISPLAYID As String = "MASTERPAGEFOOTER"       ''マスターページフッター文言取得ID
        ...
        Private Const C_CUSTOMER_SEARCHTYPE_SOCIALNUM = "6"                         ''顧客検索条件(国民番号)
        ...
        '2014/08/29 TCS 武田 Next追加要件 START
        Private Const C_DMS_CODE_TYPE_BRANCH As String = "2"                        ''基幹コード区分:店舗(2)
        Private Const C_TDIS_URL As String = "TDIS_URL"                             ''TDISサイトURL
        '2014/08/29 TCS 武田 Next追加要件 END

        '2015/12/14 TCS 外崎 (ﾄﾗｲ店ｼｽﾃﾑ評価)新車ﾀﾌﾞﾚｯﾄ(受注後)の管理機能開発 START
        Private Const C_USE_AFTER_ODR_PROC_FLG As String = "USE_AFTER_ODR_PROC_FLG"     ''セールスタブレットの受注後工程機能の利用有無
        Private Const C_USE_AFTER_ODR_PROC_FLG_ENABLED As String = "1"                  ''1:セールスタブレットの受注後工程機能を利用する
        Private Const C_USE_AFTER_ODR_PROC_FLG_DISABLED As String = "0"                 ''0:セールスタブレットの受注後工程機能を利用しない
        '2015/12/14 TCS 外崎 (ﾄﾗｲ店ｼｽﾃﾑ評価)新車ﾀﾌﾞﾚｯﾄ(受注後)の管理機能開発 END

        '2016/04/26 TCS 山口 （トライ店システム評価）他システム連携における複数店舗コード変換対応 START
        Private Const C_DMS_PROGRAM_SETTING_PROGRAM_CD As String = "COMMON"                 ''プログラム設定検索条件（プログラムコード）
        Private Const C_DMS_PROGRAM_SETTING_SETTING_SECTION As String = "TDIS"              ''プログラム設定検索条件（設定セクション）
        Private Const C_DMS_PROGRAM_SETTING_SETTING_KEY As String = "DMS_CODE_MAP_BRN_CD"   ''プログラム設定検索条件（設定キー）
        Private Const C_DMS_CODE_MAP_DMS_CD_2 As String = "DMS_CD_2"                        ''使用基幹コード(基幹コード2)
        Private Const C_DMS_CODE_MAP_DMS_CD_3 As String = "DMS_CD_3"                        ''使用基幹コード(基幹コード3)
        ...
        '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 START
        Private Const C_PP_BUTTON_ID As Short = 1800
        '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 END
...
#Region "OnInit"
        ...
        Protected Overrides Sub OnInit(e As System.EventArgs)
            ...
            '2012/11/26 TCS 河原 Aカード情報相互連携開発 START
            'ショールームステータスを追加。
            If (VersionInformation.IsEqualOrLaterThan(1, 2, 0)) Then
                If (staff.OpeCD = Operation.BM OrElse staff.OpeCD = Operation.SSM OrElse staff.OpeCD = Operation.SLR OrElse staff.TeamLeader) Then
                ...
                End If
            End If
            '2012/11/26 TCS 河原 Aカード情報相互連携開発 END

            ''顧客情報を追加。
            If (staff.OpeCD <> Operation.TEC AndAlso staff.OpeCD <> Operation.SLR) Then
            ...
            '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 START
            If (staff.OpeCD <> Operation.SS Or staff.OpeCD <> Operation.SSM) Then
                _AddFooterButton(FooterMenuCategory.PP)
                If _category = FooterMenuCategory.PP Then
                    _AddFooterButton(buttonIds)
                End If
            End If
            '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 END
            ...
            '2012/11/16 TCS 藤井 【A.STEP2】次世代e-CRB  新車タブレット横展開に向けた機能開発 START
            'TCV設定を追加。
            '2013/01/25 TCS 上田 Dist権限の場合にフッターにTCV設定ボタンを追加 START
            'If (staff.OpeCD = Operation.SSM) Then
            If (staff.OpeCD = Operation.SSM OrElse staff.OpeCD = Operation.DM OrElse staff.OpeCD = Operation.DO) Then
                '2013/01/25 TCS 上田 Dist権限の場合にフッターにTCV設定ボタンを追加 END
                _AddFooterButton(FooterMenuCategory.TCVSetting)
                If _category = FooterMenuCategory.TCVSetting Then
                    _AddFooterButton(buttonIds)
                End If
            End If
            '2012/11/16 TCS 藤井 【A.STEP2】次世代e-CRB  新車タブレット横展開に向けた機能開発 END
            ...
            '2013/10/31 TCS 山田 ADD i-CROP再構築後の新車納車システムに追加したリンク対応 START
            ''新車納車システム連携メニューを追加。ただし、デフォルトは非表示とする
            _AddFooterButton(FooterMenuCategory.LinkMenu)
            If _category = FooterMenuCategory.LinkMenu Then
                _AddFooterButton(buttonIds)
            End If
            _footButtons(FooterMenuCategory.LinkMenu).Visible = False
            '2013/10/31 TCS 山田 ADD i-CROP再構築後の新車納車システムに追加したリンク対応 END

            '2014/03/10 TCS 武田 受注後フォロー機能開発 START
            If (staff.OpeCD = Operation.SSM OrElse staff.OpeCD = Operation.SS) Then
                _AddFooterButton(FooterMenuCategory.NewCarExplain)
                '2015/12/14 TCS 外崎 (ﾄﾗｲ店ｼｽﾃﾑ評価)新車ﾀﾌﾞﾚｯﾄ(受注後)の管理機能開発 START
                _footButtons(FooterMenuCategory.NewCarExplain).Visible = UseAfterOdrProc()
                '2015/12/14 TCS 外崎 (ﾄﾗｲ店ｼｽﾃﾑ評価)新車ﾀﾌﾞﾚｯﾄ(受注後)の管理機能開発 END
                '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 START
                'デフォルトで非活性とする
                _footButtons(FooterMenuCategory.NewCarExplain).Enabled = False
                '2018/06/22 TCS 河原 （トライ店システム評価）基幹連携を用いたセールス業務実績入力の検証 END
                If _category = FooterMenuCategory.NewCarExplain Then
                    _AddFooterButton(buttonIds)
                End If
            End If

            If (staff.OpeCD = Operation.SSM OrElse staff.OpeCD = Operation.SS OrElse staff.OpeCD = Operation.BM OrElse staff.OpeCD = Operation.ABR OrElse staff.OpeCD = Operation.SGM OrElse staff.OpeCD = Operation.OSH OrElse staff.OpeCD = Operation.OSB) Then
                _AddFooterButton(FooterMenuCategory.SPM)
                If _category = FooterMenuCategory.SPM Then
                    _AddFooterButton(buttonIds)
                End If
            End If
            '2014/03/10 TCS 武田 受注後フォロー機能開発 END
            '2014/08/29 TCS 武田 Next追加要件 START
            If (staff.OpeCD = Operation.SSM OrElse staff.OpeCD = Operation.SS OrElse staff.OpeCD = Operation.BM OrElse staff.OpeCD = Operation.OSH OrElse staff.OpeCD = Operation.OSB) Then
                _AddFooterButton(FooterMenuCategory.TDIS)
                If _category = FooterMenuCategory.TDIS Then
                    _AddFooterButton(buttonIds)
                End If
                'デフォルトは非表示
                _footButtons(FooterMenuCategory.TDIS).Visible = False
            End If
            ...
            '検索バー
            _searchBox = New CommonMasterSearchBox(CType(Me.FindControl("MstPG_CustomerSearchTextBoxPanel"), Panel), CType(Me.FindControl("MstPG_CustomerSearchTextBox"), CustomTextBox), CType(Me.FindControl("MstPG_CustomerSearchButton"), ImageButton))

            'ヘッダーのボタン、コンテキストメニュー登録
            _contextMenu = New CommonMasterContextMenu(CType(Me.FindControl("MstPG_PopOver1"), PopOver), CType(Me.FindControl("MstPG_IcropIcon"), ImageButton))
...
#Region "FooterButtonClick_EventHandler"
        ...
        Private Sub FooterButtonClick_EventHandler(ByVal sender As Object, e As EventArgs)
            ...
            '2014/03/10 TCS 武田 受注後フォロー機能開発 START
            ElseIf (CInt(id) = FooterMenuCategory.NewCarExplain) Then
                CType(Me.Page, BasePage).RedirectNextScreen("SC3280101")

            ElseIf (CInt(id) = FooterMenuCategory.SPM) Then
                CType(Me.Page, BasePage).RedirectNextScreen("SC3120201")
            '2014/03/10 TCS 武田 受注後フォロー機能開発 END
            End If
...
#Region "Page_Load"
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If (StaffContext.Current.OpeCD = Operation.SSM OrElse StaffContext.Current.OpeCD = Operation.DM OrElse StaffContext.Current.OpeCD = Operation.DO) Then
                '2013/01/25 TCS 上田 Dist権限の場合にフッターにTCV設定ボタンを追加 END
                _footButtons(FooterMenuCategory.TCVSetting).Visible = False
            End If
            '2012/11/16 TCS 藤井 【A.STEP2】次世代e-CRB  新車タブレット横展開に向けた機能開発 END
...
#Region "_SetControlWord"
        ...
        Private Sub _SetControlWord()
            ...
            For Each type As ListItem In types.Items
                ...
                Select Case type.Value
                    ...
                    Case C_CUSTOMER_SEARCHTYPE_RO
                        ...
                        '2013/12/20 TCS 市川 Aカード情報相互連携開発 追加要望 START
                    Case C_CUSTOMER_SEARCHTYPE_SOCIALNUM
                        type.Text = HttpUtility.HtmlEncode(WebWordUtility.GetWord(C_MSTPG_DISPLAYID, 24))
                        '2013/12/20 TCS 市川 Aカード情報相互連携開発 追加要望 END
                End Select
            ...
            '2013/12/20 TCS 市川 Aカード情報相互連携開発 追加要望 START
            CType(Me.FindControl("MstPG_CustomerSearchTypeWordSocialNumTextBox"), TextBox).Text = WebWordUtility.GetWord(C_MSTPG_DISPLAYID, 25)
            '2013/12/20 TCS 市川 Aカード情報相互連携開発 追加要望 END
...
#End Region
        ...
        Private Sub Page_PreRender(sender As Object, e As System.EventArgs) Handles Me.PreRender
            ...
            If VersionInformation.IsEqualOrLaterThan(1, 1, 0) Then
                ...
                Else
                    ...
                    '2013/12/27 TCS 市川 Aカード情報相互連携開発 START
                    'チームリーダーは管理者用通知一覧も表示する。
                    If staff.TeamLeader Then
                        Me.Page.ClientScript.RegisterStartupScript(Me.GetType, "noticeListFrame", "<script type='text/javascript'>$(function(){{ $('body').append(""<div id='noticeListFrame' name='noticeListFrame'><iframe id='noticeListiFrame' name='noticeListiFrame' src='SC3040802.aspx'></iframe><div>""); }});</script>")
                    End If
                    '2013/12/27 TCS 市川 Aカード情報相互連携開発 END
                End If
            Else
                forumButtonPanel.Visible = False
            End If
            ...
            '商品紹介ボタン押下時のイベント
            GetFooterButton(C_PP_BUTTON_ID).OnClientClick = "location.href = 'icrop:product-explanation://'"
        ...
        ''' <summary>
        ''' セールスタブレットの受注後工程機能の利用有無の設定を取得
        ''' </summary>
        ''' <returns>True:利用する False:利用しない</returns>
        ''' <remarks>USE_AFTER_ODR_PROC_FLG_DISABLED</remarks>
        Public Function UseAfterOdrProc() As Boolean
            Dim dlrSetting As New SystemSettingDlr
            Dim staffContext As StaffContext = staffContext.Current
            Dim setting As SystemSettingDlrDataSet.TB_M_SYSTEM_SETTING_DLRRow = dlrSetting.GetEnvSetting(staffContext.DlrCD, staffContext.BrnCD, C_USE_AFTER_ODR_PROC_FLG)
            If (setting IsNot Nothing AndAlso setting.SETTING_VAL = C_USE_AFTER_ODR_PROC_FLG_DISABLED) Then
                Return False
            Else
                Return True
            End If
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Sub DisplayTDISFooterButton(ByVal param As TDISFooterButton)
            Dim staff As StaffContext = StaffContext.Current
            Dim sysenv As New SystemEnvSetting
            Dim tdisUrlRow As SystemEnvSettingDataSet.SYSTEMENVSETTINGRow = sysenv.GetSystemEnvSetting(C_TDIS_URL)

            '対象権限の場合かつ、TDISURLが存在する場合、フッターを表示にする
            If (staff.OpeCD = Operation.SSM OrElse staff.OpeCD = Operation.SS OrElse staff.OpeCD = Operation.BM OrElse staff.OpeCD = Operation.OSH OrElse staff.OpeCD = Operation.OSB) And (tdisUrlRow IsNot Nothing) Then
                If Not String.IsNullOrWhiteSpace(tdisUrlRow.PARAMVALUE) Then
                    'フッターを表示
                    _footButtons(FooterMenuCategory.TDIS).Visible = True

                    '販売店コードと店舗コードをDMS変換する
                    Dim dmsCodeMap As New DmsCodeMap
                    Dim dmsCodeMapRow As DmsCodeMapDataSet.DMSCODEMAPRow = dmsCodeMap.GetDmsCodeMap(C_DMS_CODE_TYPE_BRANCH, param.DlrCD, param.BrnCD)

                    'URLを生成する
                    Dim tdisUrl As String = tdisUrlRow.PARAMVALUE

                    Dim url As New StringBuilder
                    url.Append(tdisUrl)

                    '機能ID(SG0101)
                    '2016/01/07 TCS 市川 BTS-229(TDIS仕様変更) START
                    url.Append("?TARGET_ID=SG0101")
                    '2016/01/07 TCS 市川 BTS-229(TDIS仕様変更) END

                    '2016/01/07 TCS 市川 BTS-229(TDIS仕様変更) START
                    If Not dmsCodeMapRow Is Nothing Then
                        '販売店コード                                
                        Dim dlrCd As String = dmsCodeMapRow.DMS_CD_1.PadRight(5)
                        url.Append("&DEALER_CD=" + HttpUtility.UrlEncode(dlrCd))

                        '2016/04/26 TCS 山口 （トライ店システム評価）他システム連携における複数店舗コード変換対応 START
                        '店舗コード
                        'プログラム設定より使用するTDISで使用するDMS店舗コードを選択する
                        Dim programSetting As New ProgramSettingV4
                        Dim programSettingRow As ProgramSettingV4DataSet.PROGRAMSETTINGV4Row =
                                                 programSetting.GetProgramSettingV4(C_DMS_PROGRAM_SETTING_PROGRAM_CD,
                                                                                    C_DMS_PROGRAM_SETTING_SETTING_SECTION,
                                                                                    C_DMS_PROGRAM_SETTING_SETTING_KEY)

                        Dim brnCd As String = String.Empty
                        If Not programSettingRow Is Nothing Then
                            Dim dmsBrnCdSetting As String = programSettingRow.SETTING_VAL
                            If C_DMS_CODE_MAP_DMS_CD_3.Equals(dmsBrnCdSetting) Then
                                brnCd = dmsCodeMapRow.DMS_CD_3.PadRight(3)
                            Else
                                brnCd = dmsCodeMapRow.DMS_CD_2.PadRight(3)
                            End If
                        Else
                            brnCd = dmsCodeMapRow.DMS_CD_2.PadRight(3)
                        End If
                        ...
                        '2016/04/26 TCS 山口 （トライ店システム評価）他システム連携における複数店舗コード変換対応 END
                        url.Append("&BRANCH_CD=" + HttpUtility.UrlEncode(brnCd))
                    End If
                    '2016/01/07 TCS 市川 BTS-229(TDIS仕様変更) END

                    'ユーザーID
                    Dim userId As String = ""
                    Dim accountTokens As String() = StaffContext.Current.Account.Split(New Char() {"@"c}, 2)
                    If (1 <= accountTokens.Length) Then
                        userId = accountTokens(0)
                    End If
                    '2016/01/07 TCS 市川 BTS-229(TDIS仕様変更) START
                    url.Append("&USER_ID=" + HttpUtility.UrlEncode(userId))
                    '2016/01/07 TCS 市川 BTS-229(TDIS仕様変更) END

                    Logger.Info("DisplayTDISFooterButton URL:[" + url.ToString + "]")

                    _footButtons(FooterMenuCategory.TDIS).OnClientClick = "return displayMaxSizeDialog('" + url.ToString + "');"
                End If
            End If
        End Sub
        '2014/08/29 TCS 武田 Next追加要件 END
