                    ...
                    .bind("click." + pluginName, function (e) {
                        ...
                        if ($target.innerWidth() < w) {
                            ...
                            if ($target.innerWidth() - rw <= x) {
                                ...
                                //チップ表示位置指定
                                var tipX = $target.offset().left + 30;
                                var tipY = $target.offset().top - $tip.outerHeight() - 12;
                                ...
                                $tip.fadeIn(400, function () {
                                    ...
                                    $(document).bind("select mousedown touchstart", function (e) {
                                        if (!$(e.target).is($tip)) {
                                            //チップエリア以外をクリックした場合、チップを閉じる
                                            clearInterval(t);
                                            $tip.fadeOut(400, function () {
                                                $tip.remove();
                                            });
                                            //イベントのバインド解除
                                            $(document).unbind("select mousedown touchstart", arguments.callee);
                                        }
                                    });
                                });
