    ...
    pluginImpl.prototype = {
        ...
        create: function (options, elem) {
            ...
            this.originalElement
                .bind("mousedown touchstart", data, function (e) {
                    var dragArgs = e.data,
                        innerPosition = self.inner.position(),
                        rewindTop = {
            ...
            this.progress = $("<div class='icrop-CustomRepeater-progress' style='position:absolute;top:0px;left:0px;'><div style='position:absolute;' class='icrop-CustomRepeater-progress-inner'><div style='position:absolute;' class='icrop-CustomRepeater-progress-inner-icon'></div></div></div>").appendTo(element);
            this.progress
                .width(element.width())
                .height(element.height());

            progressInner = this.progress.find(".icrop-CustomRepeater-progress-inner");
            progressInner.css({
                top: ((element.height() / 2) - (progressInner.height() / 2)),
                left: ((element.width() / 2) - (progressInner.width() / 2))
            });
            progressIcon = this.progress.find(".icrop-CustomRepeater-progress-inner-icon");
            progressIcon.css({
                top: ((progressInner.height() / 2) - (progressIcon.height() / 2)),
                left: ((progressInner.width() / 2) - (progressIcon.width() / 2))
            });

            this.inner = $('<div class="icrop-CustomRepeater-inner" style="position:relative;" />').appendTo(element);
            ...
            this.inner.css('top', (this.innerTop.height() + this.innerTopPager.height() - 10) * -1);
        ...
        mousemove: function (e) {
            ...
            if (dragArgs.mode == "rewind") {
                ...
                self.innerTopPager.children('.icrop-CustomRepeater-inner-topPager-icon').show().css("-webkit-transform", "rotate(" + degree + "deg)");
                ...
                self.innerBottomPager.children('.icrop-CustomRepeater-inner-bottomPager-icon').show().css("-webkit-transform", "rotate(" + degree + "deg)");
            } else {
                //noop
            }

            self.inner.css('top', currentTop);
        ...
        mouseup: function (e) {
            ...
            if (dragArgs.mode == "do rewind") {
                self.inner.animate({ top: dragArgs.beginTop }, 100, 'linear', function () {
                    self.firstPage -= 1;
                    self.renderRewind();
                });
                ...
                self.scrollTimer = setInterval(function () {
                    ...
                    accelTop = self.inner.position().top + self.accelY;

                    if (accelTop < limitTop.min) {
                        self.inner.css('top', limitTop.min + self.accelY);
                    } else if (limitTop.max < accelTop) {
                        self.inner.css('top', limitTop.max + self.accelY);
                    } else {
                        self.inner.css('top', accelTop);
                    }
        ...
        renderForward: function (loadedCache) {
            ...
            if (this.cacheBottomIndex < targetBottomIndex) {
                ...
                if (loadedCache === undefined) {
                    loadedCache = this.options.load.call(this.elem, this, ((this.cacheBottomIndex < 0) ? targetTopIndex : (this.cacheBottomIndex + 1)), false);
                }
            ...
            //改ページ位置に移動
            this.inner.css("top", this.inner.position().top - innerTopPagerDelta + innerHeightRemoved);
            this.inner.animate({ top: this.inner.position().top - 10 }, 500, 'linear');
        ...
        renderRewind: function (loadedCache) {
            ...
            if (targetTopIndex < this.cacheTopIndex) {
                //データ取得
                if (loadedCache === undefined) {
                    loadedCache = this.options.load.call(this.elem, this, ((this.cacheTopIndex < 0) ? targetBottomIndex : (this.cacheTopIndex - 1)), true);
                }
                if (loadedCache === false) {
                    //非同期ロード
                    return result;
            ...
            if (targetBottomIndex < this.cacheTopIndex) {
                ...
                //部分行キャッシュヒット（前ページなし）
                this.renderRewindRows(targetTopIndex, this.cacheBottomIndex, false);
                result = true;
            } else {
                //全行キャッシュヒット（前ページありかも）
            ...
            //改ページ位置に移動
            this.inner.css("top", this.inner.position().top - innerHeightAdded);
            this.inner.animate({ top: this.inner.position().top + 10 }, 500, 'linear');
        ...
        resizeScrollBar: function (data) {
            ...
            data.scrollBar
                .height(Math.max(Math.ceil(scrollH * rate), 20))
                .css('top', top);
