...
'更新： 2015/11/10 TM t.komure  (トライ店システム評価)SMBチップ検索の絞り込み方法変更 $03
'更新： 2015/12/17 TM y.nakamura ゲートキーパーのユーザ表示対応 $04
'──────────────────────────────────
...
#Region "車両登録番号検索ワード変換"
    ...
    ''' <param name="regNumDlmtr">区切り文字</param>
    ''' <returns>「*」と区切り文字を取り除いた検索ワード</returns>
    ''' <remarks></remarks>
    Public Function ConvertVclRegNumWord(ByVal inSearchWord As String, Optional ByVal regNumDlmtr As String = Nothing) As String
        ...
        '区切り文字を取得
        ' $04 start ゲートキーパーのユーザ表示対応
        If regNumDlmtr Is Nothing then
            regNumDlmtr = GetSystemSettingValueBySettingName(SysRegNumDlmtr)
        End If       
        ' $04 end   ゲートキーパーのユーザ表示対応
