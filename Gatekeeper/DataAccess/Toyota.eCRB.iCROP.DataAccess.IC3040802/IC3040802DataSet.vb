...
#Region "定数"
        ...
        ''' 未読
        ''' </summary>
        Private Const Unread As String = "0"
        ''' <summary>
        ''' 通知依頼種別：来店
        ''' </summary>
        Private Const ComingStore As String = "04"
#End Region
...
#Region "通知未読件数取得処理"
        ...
        ''' <param name="sendDate">ユーザーID</param>
        ''' <returns>通知未読件数</returns>
        ''' <remarks></remarks>
        Public Function SelectUnreadNotice(ByVal account As String, ByVal sendDate As Date) As Long
            Logger.Info(getLogMethod(System.Reflection.MethodBase.GetCurrentMethod.Name, True))
            Logger.Info(getLogParam("account", account, False))
            Logger.Info(getLogParam("sendDate", CStr(sendDate), True))
            ...
            Using query As New DBSelectQuery(Of IC3040802DataSet.IC3040802UnReadNoticeCountDataTable)("IC3040802_001")
                ...
                With sql
                    ...
                    .Append("    AND T1.NOTICEREQCTG <> :NOTICEREQCTG")
                    .Append("    AND T2.READFLG = :READFLG")
                    .Append("    AND T2.TOACCOUNT = :TOACCOUNT")
                    .Append("    AND T2.SENDDATE >= TRUNC(:SENDDATE)")
                End With
                query.CommandText = sql.ToString()
                'SQLパラメータ設定
                query.AddParameterWithTypeValue("NOTICEREQCTG", OracleDbType.Char, ComingStore)
                query.AddParameterWithTypeValue("READFLG", OracleDbType.Char, Unread)
                ...
                Dim executeNumber As Integer = dt.Count
