...
'更新： 2013/9/5   TMEJ t.shimamura 新車受付係りによる来店チップ作成機能開発 $03
'更新： 2013/10/15 TMEJ m.asano   次世代e-CRBセールス機能 新DB適応に向けた機能開発 $04
'更新： 2014/01/17 TMEJ 陳   TMEJ次世代サービス 工程管理機能開発 $05
'更新： 2015/11/10 TM t.komure    (トライ店システム評価)SMBチップ検索の絞り込み方法変更 $06
'──────────────────────────────────
...
#Region "顧客一覧取得"
        ...
        ''' </summary>
        ''' <param name="dealerCode">販売店コード</param>
        ''' <param name="searchType">検索タイプ(1：車両登録No 、2：顧客名称、 3：VIN、 4：電話番号/携帯番号)</param>
        ...
        Public Function GetCustomerList(ByVal dealerCode As String, _
            ...
            With sql
                ...
                .Append("      , CUST.SACODE ")
                .Append("      , CUST.CUSTYPE ")
                ' $05 STARTTMEJ次世代サービス 工程管理機能開発
                .Append("      , CUST.REG_AREA_NAME ")
                ' $05 END  TMEJ次世代サービス 工程管理機能開発
                .Append(" FROM ( ")
                .Append("         SELECT ")
                ...
                ' $05 STARTTMEJ次世代サービス 工程管理機能開発
                .Append("           AND VCL_DLR.REG_AREA_CD = AREA.REG_AREA_CD(+) ")
                ' $05 END  TMEJ次世代サービス 工程管理機能開発

                '1：車両登録No 、2：顧客名称、 3：VIN、 4：電話番号/携帯番号
            ...
            Using query As New DBSelectQuery( _
                ...
                Select Case searchType
                    Case SearchTypeRegNumber        '車両登録No
                        ...
                        Next
                    Case SearchTypeCustomerName     '顧客名称
                        query.AddParameterWithTypeValue("NAME", OracleDbType.NVarchar2, searchText(0).ToUpper(CultureInfo.CurrentCulture))
                    Case SearchTypeVin              'VIN
...
#End Region
        ...
        ' $06 START (トライ店システム評価)SMBチップ検索の絞り込み方法変更
#Region "システム設定値取得"
...
#End Region
        ' $06 END (トライ店システム評価)SMBチップ検索の絞り込み方法変更

    End Class
End Namespace
Partial Class VisitReceptionDataSet
    Partial Class VisitReceptionCustomerListDataTable
