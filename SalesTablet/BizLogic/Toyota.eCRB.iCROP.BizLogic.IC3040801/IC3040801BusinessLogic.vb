...
'更新： 2012/11/05 TMEJ 嶋村 サービス入庫通知を追加 $02
'更新:  2012/05/29 TMEJ 嶋村【A.STEP2】次世代e-CRB新車タブレット　新DB適応に向けた機能開発 $03
'更新:  2014/01/14 TMEJ 嶋村 セールスタブレット契約承認機能開発 $04
'更新:  2014/03/03 TMEJ 中村 受注後フォロー機能開発 $05
'更新:  2014/04/08 TMEJ 中村 納車予定日変更対応 $06
'更新:  2014/06/30 TMEJ 皆川 フォローアップメモ更新対応 $07
'─────────────────────────────────────
...
#Region "共通定数"
    ...
    ''' <summary>AfterOdrActCd要素</summary>
    Private Const XmlDataAfterOdrActCd As String = "AfterOdrActCd"
    ' $05 end 受注後フォロー機能開発
    ...
    Private Enum Status As Integer
        ...
        ' $04 start ステータス区分追加変更対応
        ''' <summary>回答(承認)</summary>
        AcceptanceStatus = 4

        ''' <summary>回答(否認)</summary>
        RejectStatus = 5
    ...
    ''' <summary>通知依頼種別：注文情報登録・変更</summary>
    Private Const NoticeClassOrderInformationRegistrationAndChange As String = "09"
    ' $04 end　契約承認機能追加対応

    ' $05 start 受注後フォロー機能開発
    ''' <summary>通知依頼種別：受注後フォロー</summary>
    Private Const NoticeClassAfterOdrFollow As String = "10"
    ' $05 end 受注後フォロー機能開発

    ' $06 start 納車予定日変更対応
    ''' <summary>通知依頼種別：納車予定日変更</summary>
    Private Const NoticeClassDeliScheDateChg As String = "11"
    ' $06 end 納車予定日変更対応

    ' $07 start フォローアップメモ更新対応
    ''' <summary>通知依頼種別：フォローアップメモ更新</summary>
...
#End Region
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const ConstraintLog As Boolean = True
    ...
    Private Sub GetRequestNoticeElementValue(ByVal detailClone As XmlNode,
        ...
        Select Case noticeDisposalMode
            Case NoticeDisposal.Peculiar
                ...
                ' $04 start 通知依頼ID： 依頼種別が08(契約承認)の場合、ステータスが1(依頼)以外は必須
                If Not CStr(Status.RequestStatus).Equals(requestNoticeData.Status) _
                    And (NoticeClassAssessment.Equals(requestNoticeData.RequestClass) _
                         ...
                         OrElse NoticeClassHelp.Equals(requestNoticeData.RequestClass) _
                         OrElse NoticeClassContractApprovalRequest.Equals(requestNoticeData.RequestClass)) Then
                    ' $04 end
                ...
                ' $04 start 依頼種別IDは「依頼種別=08(契約承認依頼、09(注文情報変更)))」の場合、必須
                If NoticeClassAssessment.Equals(Me.requestNoticeData.RequestClass) OrElse
                   ...
                   NoticeClassCSSurvey.Equals(Me.requestNoticeData.RequestClass) OrElse
                   NoticeClassContractApprovalRequest.Equals(Me.requestNoticeData.RequestClass) OrElse
                   NoticeClassOrderInformationRegistrationAndChange.Equals(Me.requestNoticeData.RequestClass) Then
                    ' $01 end   step2開発
                ...
                Me.requestNoticeData.RequestClassId = CDec(requestClassIdText)
                ' $03 end 桁数変更対応

                ' $01 start step2開発                
                ' お客様名ID、お客様名、顧客分類、顧客種別は「ステータス=1(依頼)」の場合は必須
                ' 依頼種別が05(苦情),06(CS Survey)の場合、ステータス値に関わらず必須
                ' $02 start 依頼種別が07(サービス入庫)の場合、ステータス値に関わらず必須
                ' $04 start 依頼種別が09(注文情報登録・変更)の場合、ステータス値に関わらず必須
                ' $05 start 依頼種別が10(受注後フォロー)の場合、ステータス値に関わらず必須
                ' $06 start 依頼種別が11(納車予定日変更)の場合、ステータス値に関わらず必須
                ' $07 start 依頼種別が12(フォローアップメモ更新)の場合、ステータス値に関わらず必須
                ...
                If CStr(Status.RequestStatus).Equals(Me.requestNoticeData.Status) _
                   ...
                   OrElse NoticeClassCSSurvey.Equals(requestNoticeData.RequestClass) _
                   OrElse NoticeClassSurviceStore.Equals(requestNoticeData.RequestClass) _
                   OrElse NoticeClassOrderInformationRegistrationAndChange.Equals(requestNoticeData.RequestClass) _
                   OrElse NoticeClassAfterOdrFollow.Equals(requestNoticeData.RequestClass) _
                   OrElse NoticeClassDeliScheDateChg.Equals(requestNoticeData.RequestClass) _
                   OrElse NoticeClassFllwupMemoUpdate.Equals(requestNoticeData.RequestClass) Then
                    ' $07 end 依頼種別が12(フォローアップメモ更新)の場合、ステータス値に関わらず必須
                    ' $06 start 依頼種別が11(納車予定日変更)の場合、ステータス値に関わらず必須
                    ' $05 start 依頼種別が10(受注後フォロー)の場合、ステータス値に関わらず必須
                    ' $02 end 依頼種別が07(サービス入庫)の場合、ステータス値に関わらず必須
                    ' $01 end   step2開発
                ...
                Me.requestNoticeData.FollowUpBoxNumber = CDec(followUpBoxNumberText)
                ...
                '$05 start 受注後フォロー機能開発
                Dim afterOdrActCdDataAssignment As DataAssignment
                If NoticeClassAfterOdrFollow.Equals(requestNoticeData.RequestClass) Then
                    afterOdrActCdDataAssignment = DataAssignment.ModeMandatory
                Else
                    afterOdrActCdDataAssignment = DataAssignment.ModeOptional
                End If

                Me.requestNoticeData.AfterOdrActCd = GetNodeInnerText(requestNoticeXml,
                                                XmlDataAfterOdrActCd,
                                                afterOdrActCdDataAssignment,
                                                2,
                                                TypeConversion.StringType,
                                                ElementName.AfterOdrActCd)
                '$05 end 受注後フォロー機能開発
    ...
    Private Sub RegistsNoticeDB(ByVal xmlDataClass As XmlNoticeData,
        ...
        Try
            Using Me.daDataSetTableAdapters
                ...
                ' $01 end   step2開発
                ' $05 start 受注後フォロー機能開発
                If String.IsNullOrWhiteSpace(xmlDataClass.RequestNotice.AfterOdrActCd) then
                    drRequest.AFTER_ODR_ACT_CD = " "
                Else
                    drRequest.AFTER_ODR_ACT_CD = xmlDataClass.RequestNotice.AfterOdrActCd
                End If
                ' $05 end 受注後フォロー機能開発
                ...
                drInfo.READFLAG = CStr(Read.Unread)
                ...
                If selfAddFlag = False _
                    ...
                    drInfo.READFLAG = CStr(Read.Read)
                    If Not String.IsNullOrEmpty(xmlDataClass.RequestNotice.FromAccount) Then
                        drInfo.ACCOUNT = xmlDataClass.RequestNotice.FromAccount
                    Else
                        drInfo.ACCOUNT = xmlDataClass.RequestNotice.FromClientId
...
#Region "値チェック"
    ...
    Private Sub CheckXmlDataClass(ByVal xmlDataClass As XmlNoticeData,
        ...
        If noticeDisposalMode = NoticeDisposal.Peculiar Then
            ...
            '通知依頼ID： 依頼種別が01(査定),02(価格相談),03(ヘルプ)の場合、ステータスが1(依頼)以外は必須
            '$04 start
            ' 依頼ID:08(契約承認依頼の場合、ステータスが1(依頼)以外は必須
            ...
            If Not CStr(Status.RequestStatus).Equals(xmlDataClass.RequestNotice.Status) _
                And (CStr(NoticeClassAssessment).Equals(xmlDataClass.RequestNotice.RequestClass) _
                     ...
                     OrElse CStr(NoticeClassHelp).Equals(requestNoticeData.RequestClass) _
                     OrElse CStr(NoticeClassContractApprovalRequest).Equals(requestNoticeData.RequestClass)) Then
                ' $01 end   step2開発
                ' $04 end
            ...
            '依頼種別IDは「依頼種別=01(査定) or 依頼種別=02(価格相談) or 06(CS Survey)」の場合、必須
            ' $04 start
            '依頼種別IDは[08(契約承認依頼) or 09(注文情報登録・変更)」の場合も必須
            If NoticeClassAssessment.Equals(xmlDataClass.RequestNotice.RequestClass) Or
               ...
               NoticeClassCSSurvey.Equals(xmlDataClass.RequestNotice.RequestClass) Or
               NoticeClassContractApprovalRequest.Equals(xmlDataClass.RequestNotice.RequestClass) Or
               NoticeClassOrderInformationRegistrationAndChange.Equals(xmlDataClass.RequestNotice.RequestClass) Then
                ' $01 end   step2開発
                ' $04 end
            ...
            ' $02 start 依頼種別が07(サービス入庫)の場合、ステータス値に関わらず必須
            ' $04 start 
            ' 依頼種別が09(注文情報登録・変更)の場合、ステータス値に関わらず必須
            If CStr(Status.RequestStatus).Equals(xmlDataClass.RequestNotice.Status) _
                ...
                OrElse NoticeClassCSSurvey.Equals(xmlDataClass.RequestNotice.RequestClass) _
                OrElse NoticeClassSurviceStore.Equals(xmlDataClass.RequestNotice.RequestClass) _
                OrElse NoticeClassOrderInformationRegistrationAndChange.Equals(xmlDataClass.RequestNotice.RequestClass) Then
                ' $02 end   依頼種別が07(サービス入庫)の場合、ステータス値に関わらず必須
                ' $01 end   step2開発
                ' $04 end
                'お客様名ID
...
#Region "PushServer処理"
    ...
    Private Sub SendPushServer(ByVal xmlDataClass As XmlNoticeData)
        ...
        '$03 start  CS Surveyのキャンセル処理はPush送信しない
        If String.IsNullOrEmpty(xmlDataClass.RequestNotice.PushInfo) _
         OrElse NoticeClassCSSurvey.Equals(xmlDataClass.RequestNotice.RequestClass) _
         And CStr(Status.CancelStatus).Equals(xmlDataClass.RequestNotice.Status) Then
            Return
        End If
        '$03 end  CS Surveyのキャンセル処理はPush送信しない
        ...
        'メッセージが空文字の場合は処理しない
        If Validation.Equals(pushMessage, String.Empty) then
            Me.errorInfo.ResultId = CreateResultId(ReturnCode.XmlValueCheckError, ElementName.DisplayContents)
            Throw New ArgumentException()
        End If
...
#Region "受付メイン・画面更新用のPushServer処理のメソッド"
    ...
    Private Sub SendPushServerForRefresh(ByVal requestClass As String, ByVal currentStatus As String,
        ...
        Using usersDt As VisitUtilityUsersDataTable = GetPushUser(dealerCode, storeCode, operationCdListForSSV)
            ...
            For Each dr As VisitUtilityUsersRow In usersDt
                'USERを置換して、Push送信する
                visitUtility.SendPushPC(Replace(pushMessageForSSV, ReplaceAccount, dr.ACCOUNT), dealerCode)
            Next dr
        End Using
