...
Public Class SC3070301BusinessLogic
    ...
    Public Function UpdateConstractInfoSend(ByVal tbl As SC3070301DataSet.SessionDataTable,
        ...
        Try
            ...
            '通知キャンセル
            If Not UpdateNoticeRequest(tblDataRow.ESTIMATEID) Then
                commit = False
            End If
        ...
        Using dt As New SC3070301DataSet.ConstractInfoDataTable
            ...
            For Each payment As IC3070201PaymentInfoRow In apiDt.Tables("IC3070201PaymentInfo").Rows()
                If method.Equals(payment.Item("PAYMENTMETHOD")) Then
                    ...
                    '2012/07/30 TCS 高橋  兆方店展開 START
                    If (Not IsDBNull(payment.Item("FINANCECOMNAME"))) AndAlso String.IsNullOrEmpty(rtnDt.FINANCECOMNAME) Then
                        'ローン会社（融資会社名）
                        rtnDt.FINANCECOMNAME = payment.FINANCECOMNAME
                    End If
                    '2012/07/30 TCS 高橋  兆方店展開 END
                End If
            Next
            ...
            '2012/07/30 TCS 高橋  兆方店展開 START
            Dim assessedprice As Double = 0
            For Each tradein As IC3070201TradeincarInfoRow In apiDt.Tables("IC3070201TradeincarInfo").Rows()
                assessedprice = assessedprice + tradein.ASSESSEDPRICE
            Next
            '下取金額
            rtnDt.TRADEINPRICE = assessedprice
            '2012/07/30 TCS 高橋  兆方店展開 END
    ...
    ''' 通知キャンセル処理
    ''' </summary>
    ''' <param name="estimateId">見積りID</param>
    ''' <returns>処理結果</returns>
    ''' <remarks>通知キャンセル処理</remarks>
    Public Function UpdateNoticeRequest(ByVal estimateId As Long) As Boolean

        Logger.Info("UpdateNoticeRequest Start")

        'キャンセル対象件取得
        Dim NoticeRequestDt As SC3070301DataSet.NoticeRequestDataTable
        NoticeRequestDt = SC3070301TableAdapter.GetNoticeRequest(estimateId)

        If NoticeRequestDt.Count = 1 Then

            Dim rsltId As Integer
            Dim returnXmlNotice As XmlCommon
            Dim NoticeRequestRw As SC3070301DataSet.NoticeRequestRow
            NoticeRequestRw = CType(NoticeRequestDt.Rows(0), SC3070301DataSet.NoticeRequestRow)
            ...
            '通知登録API呼び出し
            returnXmlNotice = SetNoticeInfo(NoticeRequestRw.NOTICEREQID, estimateId, toAccount)
    ...
    Public Function SetNoticeInfo(ByVal noticereqId As Long, ByVal estimateId As Long, ByVal toAccount As String) As XmlCommon
        ...
        Using noticeData As New XmlNoticeData
            ...
            Using requestNotice As New XmlRequestNotice
                ...
                requestNotice.RequestClassId = estimateId                               '依頼ID
            ...
            Using pushInfo As New XmlPushInfo
                'pushInfo.PushCategory = "2"                                             'カテゴリータイプ
                'pushInfo.PositionType = "0"                                             '表示位置
                'pushInfo.DisplayType = "4"                                              '表示タイプ
                'pushInfo.DisplayFunction = "icropScript.ui.openNoticeList()"
                ...
                pushInfo.Color = "1"                                                    '色
                pushInfo.DisplayContents = WebWordUtility.GetWord(906)                  '表示内容
