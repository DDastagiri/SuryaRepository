...
#Region "Push送信"
    ...
    Private Sub SendPushPrivate(ByVal postMsg As String, _
        ...
        Try
            ...
            Dim gateWayAddress As TB_M_SYSTEM_SETTINGRow = systemSetting.GetSystemSetting(GatewayServerDomain)
            ' 2019/11/12 NSK 鈴木 (トライ店システム評価)次世代セールス基盤：ログ出力機能における保守性向上検証 END
            ...
            ' 接続先Pushサーバアドレスを取得する
            Dim pushServerAddressURL As String = GetConnectInfo(pushServerDealerCode, CStr(gateWayAddress.SETTING_VAL))
            ' 2019/11/12 NSK 鈴木 (トライ店システム評価)次世代セールス基盤：ログ出力機能における保守性向上検証 END
