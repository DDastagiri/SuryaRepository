...
'更新：2014/03/03 TMEJ y.nakamura 受注後フォロー機能開発 $02
'
'
'
'
...
#Region "画面遷移定数"
    ...
    ''' CS Survery
    ''' </summary>
    Private Const CSSurveyDetails As String = "4"
    ' $01 end   step2開発
    ...
    ''' セッションキー(依頼種別ID)
    ''' </summary>
    Private Const SearchKeyAnswerId As String = "SearchKey.ANSWERID"
    ' $01 end   step2開発
...
#Region "メソッド"
    ...
    Protected Sub NoticeHistory(ByVal pageLoad As Boolean)
        ...
        Try
            ...
            If staffAuthority Then
                ...
                Else '履歴有り
                    ...
                    For i As Integer = 0 To RepeaterNoticeInfo.Items.Count - 1
                        ...
                        'アイコンの表示
                        If dtSalesNoticeHistory(i).ICONSPECIFICTYPE = IconSpecificTypeStyle Then
                            DirectCast(SalesReception.FindControl("Icons"),  _
                                       HtmlContainerControl).Attributes("style") _
            ...
            If beginRowIndex = NextRows Then
                ...
                End Using
                'ヘッダー件数の再取得
                javaScriptWord.Append("window.parent.icropScript.ui.setNotice();")
            End If
            'JSを呼ぶ
            ScriptManager.RegisterStartupScript(Me, _
                                    Me.GetType, _
    ...
    Protected Function SalesSetSessionValues() As String
        ...
        Select Case linkId '画面遷移先ごと残りのセッションを詰める
            ...
            Case PriceConsultation '見積画面(価格相談、契約承認、注文情報登録・変更)
                ...
                ' $01 start step2開発
            Case CSSurveyDetails    'CS Survey 詳細
                Logger.Info("SearchKeyDISPPAGE=" & CustomerDetails)
                Logger.Info("SearchKeyANSWERID=" & reqestId)
                Logger.Info("pageId=" & CustomerPageId)
                Me.SetValue(ScreenPos.Next, SearchKeyDISPPAGE, CustomerDetails) 'DISPPAGE

                '依頼種別ID(=回答ID)
                ' reqestIdという名称だが、REQCLASSIDを取得している
                Me.SetValue(ScreenPos.Next, SearchKeyAnswerId, reqestId) '依頼種別ID
                pageId = CustomerPageId 'ページID(SC3080201)
                ' $01 end   step2開発
