    ...
    $.extend(schedule, {
        ...
        */
        ajaxStart: function (callback) {
            ...
            //ページメソッドの呼び出し
            PageMethods.GetScheduleXmlText($("#isDisplayDate").val(),
                function (result, userContext, methodName) {
                    //パース
                    var doc = $($.parseXML(result));
                    $("#todoDelayCount").text($(doc).find("TotalDetailCount").text());
        ...
        */
        xmlReadFuncs: {
            ...
            detailLoopInner: function () {
                ...
                var Rslt = $(this).find("Rslt").text()
                //alert($(this).find("RsltDay").text());

                //TODOタグ分ループ
                $(this).find("VTodo").each(function () {
                    schedule.xmlReadFuncs.todoLoopInner(commonXml, scheduleInfo, $(this), Rslt);
                });
                //イベントタグ分ループ
            ...
            */
            todoLoopInner: function (commonXml, scheduleInfo, todoXml, Rslt) {

                //入庫予約はTODOが表示しない
                //if (todoXml.find("ScheduleDiv") == "1") return;

                //TodoIDがない場合表示しない
                if (todoXml.find("TodoID").text() === "") return;
                ...
                item.ProcessDiv = todoXml.find("ProcessDiv").text();
                ...
                item.CustomerName = scheduleInfo.find("CustomerName").text();
                item.ContactName = todoXml.find("ContactName").text();

                item.OdrDiv = todoXml.find("OdrDiv").text();
                item.Rslt = todoXml.find("Rslt").text();
            ...
            */
            scheduleLoopInner: function (commonXml, scheduleInfo, scheduleXml) {
                ...
                //イベントIDがない場合は表示しない
                if (scheduleXml.find("EventID").text() === "") return;
                ...
                item.processDiv = scheduleXml.find("ProcessDiv").text();
