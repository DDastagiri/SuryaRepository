...
(function ($, window, Math, undefined) {
	...
	$.fn.TCSflickable = flickable = function (options) {
	...
	flickable.prototype = {
		...
		init: function () {
			...
			if (!this.container) {
				...
				this.container = element.children().bind('mousedown.flickable', function (event) {
				...
				this.elementWrapper.bind('mousedown.flickable keydown.flickable', function () {
					self.deactivate();
				}).bind('mouseup.flickable mouseleave.flickable keyup.flickable', function () {
					self.activate();
				});
		...
		setOption: function (key, value) {
			...
			switch (key) {
				case 'cancel':
					...
					this.cancel.addClass('ui-flickable-canceled').bind('mouseenter.flickable', function () {
						self.canceled = true;
					}).bind('mouseleave.flickable', function () {
						self.canceled = false;
					});
		...
		dragStart: function (event) {
			...
			if (!this.canceled && !this.options.disabled) {
				...
				//modified!
				//drag start only if mouse was moved greater than 20px
				this.dragStartX = event.pageX;

				this.container.unbind('mousedown.flickable')
					.bind('mousemove.flickable', function (event) {
						return self.drag(event);
					}).bind('mouseup.flickable mouseleave.flickable', function (event) {
		...
		drag: function (event) {
			...
			//modified!
			if (Math.abs(event.pageX - this.dragStartX) < 50) {
				return;
			}
			...
			$.each(['x', 'y'], function (i, axis) {
				...
				//Modified:
				//Sometime incorrect event value is sent (event.clientX == 0) on Mobile Safari in iPad.
				//So guard condition below is required. If there isn't any guard, a user may see chattered flickable object when dragging it. 
				if (client != 0) {
					if (axis == 'x') {
		...
		dragStop: function (event) {
			...
			this.container.unbind('.flickable')
				.bind('mousedown.flickable', function (event) {
					return self.dragStart(event);
				}).bind('click.flickable', function (event) {
					return self.clickHandler(event);
				});
			...
			this.inertia = setInterval(function () {
				$.each({ x: 'Left', y: 'Top' }, function (axis, posName) {
					...
					if ((distance[axis] > 0 && scrollPos > destination[posName]) ||
						...
						var migrationLength = self.options.elasticConstant / 4 * distance[axis];
						var afterPosition = migrationLength + scrollPos;
						if (Math.abs(distance[axis]) < Math.abs(afterPosition)) {
							inertialVelocity[axis] = destination[posName] - scrollPos;
						} else {
							inertialVelocity[axis] = self.options.elasticConstant / 4 * distance[axis];
						}
					}
