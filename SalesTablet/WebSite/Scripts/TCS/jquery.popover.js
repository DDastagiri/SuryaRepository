...
(function ($) {
	$.fn.TCSpopover = function (options) {
		var KEY_ESC = 27;
		...
		*/
		...
		function calcPopoverPos(button) {
			...
			if ((coord.popoverDir == 'bottom') || (coord.popoverDir == 'top')) {
                ...
                //2012/03/06 TCS Myose Mod Start
                if (coord.popoverY < paddingTop) {
                    // out of the document at top
                    coord.deltaY = coord.popoverY - paddingTop;
                //if (coord.popoverY < settings.padding) {                
                //    // out of the document at top
                //    coord.deltaY = coord.popoverY - settings.padding;
                //2012/03/06 TCS Myose Mod End
		...
		function togglePopover(button, show) {
			...
			/*TCS Kamimoto mod 2012/02/13
			  if exists parent,bind click
			*/
			if (button.data("parentPopover")) {
				getParentPopOver().bind("click.popover", function (event) {
					hideOpenPopover();
				});
			}
		}
		...
		function hidePopover(button) {
			...
			/*TCS Kamimoto mod 2012/02/13
			if exists parent,unbind click
			*/			
			if (button.data("parentPopover")) {
				getParentPopOver().unbind("click.popover");
			}
		...
		function hideOpenPopover() {
			...
			while (pop) {
				...
				/*TCS Kamimoto mod 2012/02/13
				*/
				if (event && $(event.target).parents(".popover").length == 0) {
					pop = pop.data("parentPopover");
				} else {
					pop = null;
				}
		...
		function getParentPopOver() {
			if (settings.parentPopover) {
				return $("#" + $.fn.popover.openedPopup.data("parentPopover").data("popOverId"));
			} else {
				return null;
			}
		}
