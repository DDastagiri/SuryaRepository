...
#End Region
    ...
    Public Function RegistSchedule(ByVal xsData As String) As XmlDocument
        '$01 Add Start
        Logger.Info("IC3040403 RegistSchedule() Start")
        '$01 Add End
        Dim returnXML As New XmlDocument()
                       
        returnXML = ScheduleCoordination(xsData)
        '$01 Add Start
        Logger.Info("IC3040403 RegistSchedule() End")
        '$01 Add End
        Return returnXML
    End Function
    ...
    <WebMethod()> _
    Public Function RegistAfterOrderSchedule(ByVal xsData As String) As XmlDocument
        '$01 Add Start
        Logger.Info("IC3040403 RegistAfterOrderSchedule() Start")
        '$01 Add End
        Dim returnXML As New XmlDocument()
                       
        returnXML = AfterOrderScheduleCoordination(xsData)
        '$01 Add Start
        Logger.Info("IC3040403 RegistAfterOrderSchedule() End")
        '$01 Add End    
        Return returnXML
    End Function
...
#Region "Public関数"
    ...
    Public Function ScheduleCoordination(ByVal xsData As String) As XmlDocument
        '$01 Add Start
        Logger.Info("IC3040403 ScheduleCoordination() Start")
        '$01 Add End
        Dim returnXml As New XmlDocument()
        ...
        Try
            ...
            '$01 Add Start
            Logger.Info("IC3040403 ScheduleCoordination() End")
            '$01 Add End
            Return returnXml
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function AfterOrderScheduleCoordination(ByVal xsData As String) As XmlDocument
        ...
        Try
            ...
            'DebugLog
            Logger.Debug("Start AfterOrderScheduleCoordination")
            ...
            Dim xmlAfterOrderDataClass As New XmlRegistAfterOrderSchedule()

            Dim evacuationInfo As New IC3040402BusinessLogic(reqestXmlDocument)

            Dim registAfterOrderScheduleClone As XmlNode = bizClass.GetChildNode(reqestXmlDocument, XmlNameRegistAfterOrderSchedule, DataAssignment.ModeMandatory, ElementName.RegistAfterOrderSchedule).CloneNode(True)

            returnCodeList = GetRegistAfterOrderElementValue(registAfterOrderScheduleClone, xmlAfterOrderDataClass, evacuationInfo, bizClass)

            ' 戻り値のXMLを作成します。
            returnXml = CreateReturnXml(xmlAfterOrderDataClass.MessageId, xmlAfterOrderDataClass.CountryCode, returnCodeList, startDateTime)
...
#Region "Private関数"
    ...
    ''' <summary>
    ''' AfterOrderRegistElement要素内にある要素を専用のクラスに格納します。
    ''' </summary>
    ''' <param name="registAfterOrderScheduleClone"></param>
    ''' <param name="xmlRegistAfterOrderScheduleClass"></param>
    ''' <returns>戻り値のXMLを作成する際に必要なリターンコード</returns>
    ''' <remarks></remarks>
    ''' <history>2014/04/03 SKFC 渡邊 NEXT_STEP START</history>
    Private Function GetRegistAfterOrderElementValue(ByVal registAfterOrderScheduleClone As XmlNode, _
                                                     ByVal xmlRegistAfterOrderScheduleClass As XmlRegistAfterOrderSchedule, _
                                                     ByVal evacuationInfo As IC3040402BusinessLogic, _
                                                     ByVal bizClass As IC3040403BizLogic.IC3040403BusinessLogic) As List(Of Integer)

        ' Head要素内を取得します。
        xmlRegistAfterOrderScheduleClass = bizClass.GetAfterOrderHeadElementValue(registAfterOrderScheduleClone, xmlRegistAfterOrderScheduleClass)

        'スケジュール詳細要素初期化
        xmlRegistAfterOrderScheduleClass.InitialDetailList()

        ' リターンコードを格納するリスト
        Dim returnCodeList As List(Of Integer) = New List(Of Integer)

        ' Detail要素は複数ある場合があるので、ForEach文で回してListに格納する。
        For Each detailXml As XmlNode In bizClass.GetChildNode(registAfterOrderScheduleClone, XmlNameDetailName, _
                                                               DataAssignment.ModeMandatory, True, ElementName.RegistAfterOrderSchedule)

            Dim xmlAfterOrderDetailClass As New XmlAfterOrderDetail()
            ...
            Try
                ...
                ' Detail要素の値を取得します。
                xmlAfterOrderDetailClass = bizClass.GetAfterOrderDetailElementValue(detailClone, xmlAfterOrderDetailClass)
                xmlRegistAfterOrderScheduleClass.DetailList.Add(xmlAfterOrderDetailClass)

                ' 取得したDetailCodeの値をDBに格納します
                bizClass.ProcessingAfterOrderDataBase(xmlAfterOrderDetailClass)

                returnCodeList.Add(ReturnCode.Successful)
                
                Logger.Info("RETCD:[" + CType((ReturnCode.Successful + 0), String) + "]DLRCD:[" + xmlAfterOrderDetailClass.DealerCode + "]STRCD:[" + xmlAfterOrderDetailClass.BranchCode _
                                                      + "]SCHID:[" + xmlAfterOrderDetailClass.ScheduleId + "]")
            ...
            Catch ex As ApplicationException
                ...
                evacuationInfo.EvacuationScheduleInfo(xmlAfterOrderDetailClass.DealerCode, xmlAfterOrderDetailClass.BranchCode, _
                                                     Nothing, xmlAfterOrderDetailClass.ScheduleId, unregistReason)
                
                returnCodeList.Add(Integer.Parse(ex.Message))

                Logger.Info("RETCD:[" + CType(ex.Message, String) + "]DLRCD:[" + xmlAfterOrderDetailClass.DealerCode + "]STRCD:[" + xmlAfterOrderDetailClass.BranchCode _
                                                      + "]SCHID:[" + xmlAfterOrderDetailClass.ScheduleId + "]")
