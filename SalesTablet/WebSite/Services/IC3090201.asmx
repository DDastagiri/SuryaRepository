...
#Region "来店通知送信Webサービス"
        ...
        receptionDate = DateTimeFunc.Now.ToString
                ...
                Else
                    ' 来店車両実績情報の登録に成功した場合、
                    ' 門番スタッフへゲート通知をPush
                    sendGate.PushGateNotice(inputData(TagDlrCd), inputData(TagStrCd))
...
#Region "応答用XML作成"
      ...
      ' Dim transmissionDate As String = DateTimeFunc.Now.ToString(FormatDateTime, CultureInfo.InvariantCulture)
            Dim transmissionDate As String = DateTimeFunc.Now.ToString
