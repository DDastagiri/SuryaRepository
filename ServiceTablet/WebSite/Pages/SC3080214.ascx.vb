...
'更新： 2012/03/09 KN 瀧 【SERVICE_1】顧客メモポップアップを起動を修正
'更新： 2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発
...
Imports Toyota.eCRB.iCROP.BizLogic.SC3080205
        ...
        Try
            ...
            Using custDataTbl As New SC3080205DataSet.SC3080205CustDataTable
                ...
                If (mode = SC3080205BusinessLogic.ModeEdit) Then
                    ...
                    '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発 START
                    '行ロックバージョン
                    Me.CustomerLockVersion.Value = CType(custDataRow.LOCKVERSION, String)
                    '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発 END
                End If
            End Using
        Finally
            ''終了ログの出力
    ...
    Private Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            Else
                ...
                '初期表示チェック
                If "0".Equals(DirectCast(Me.InitHiddenField.Value, String)) Then
                    '初期表示の場合
                    ...
                    ' 既存客モード、新規登録モードの判定
                    If String.IsNullOrEmpty(Trim(crcustId)) = False Then
                        '既存客モード
        ...
        Try
            ...
            If ContainsKey(ScreenPos.Current, SESSION_KEY_DMSID) Then
                ...
                Using da As New IC3080204BusinessLogic()
                    Using dt As IC3080204DataSet.IC3080204CustomerDataTable = da.GetMyCustomerInfo(basicCustomerId, StaffContext.Current.DlrCD)
...
#Region "職業選択時イベント"
    ...
    Private Sub CustomerRelatedOccupationButtonRepeater_ItemCommand(ByVal sender As Object, ByVal e As RepeaterCommandEventArgs) Handles CustomerRelatedOccupationButtonRepeater.ItemCommand
        ...
        Try
            '職業登録処理
            '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
    ...
    Private Sub _RegistCustomerRelatedOccupation(ByVal OccupationNo As String, ByVal otherOccupation As String, ByVal selectedFlg As String)
        ...
        Try
            Using params As New SC3080201DataSet.SC3080201InsertCstOccupationDataTable '検索条件格納用
                ...
                End If
                '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
                paramsDr.ROWLOCKVERSION = CType(Me.CustomerLockVersion.Value, Long)
                '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発END

                params.Rows.Add(paramsDr)

                Dim bizClass As New SC3080201BusinessLogic
                '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
                'bizClass.InsertCstOccupation(params)
                If bizClass.InsertCstOccupation(params) Then
                    '成功した場合は行ロックバージョンを加算する
        ...
        Try
            Using dt As New SC3080201DataSet.SC3080201InsertCstFamilyDataTable
                ...
                For i As Integer = 0 To CInt(Me.FamilyCount.Value) - 1
                    ...
                    '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
                    '販売店店舗テーブルの行ロックバージョン取得
                    dr.ROWLOCKVERSION = CType(Me.CustomerDLRLockVersion.Value, Long)
                    '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発END
                    dt.Rows.Add(dr)
                ...
                '成功した場合は行ロックバージョンを加算する
                If bizClass.InsertCstFamily(dt) Then
...
#Region "希望連絡方法設定"
    ...
    Private Sub _SetCustomerRelatedContactPopupContactTool(ByVal contactFlgTbl As SC3080201DataSet.SC3080201ContactFlgDataTable)
        ...
        Try
            ...
            '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
            '行ロックバージョン取得
            Dim customerDealerTableRockVersion As String = CType(drContactFlg.Item("ROW_LOCK_VERSION"), String)
            '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発END
            ...
            '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
            '行ロックバージョン設定
            Me.CustomerDLRLockVersion.Value = customerDealerTableRockVersion
            '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発END
    ...
    Private Sub RegistCustomerRelatedContact()
        ...
        Try
            Using dtInfo As New SC3080201DataSet.SC3080201InsertCstContactInfoDataTable
                ...
                '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
                '行ロックバージョン取得
                drInfo.ROWLOCKVERSION = CType(Me.CustomerDLRLockVersion.Value, Long)
                '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発END
                ...
                If bizClass.InsertCstContactInfo(dtInfo) Then
                    '成功した場合は行ロックバージョンを加算する
                    Me.CustomerDLRLockVersion.Value = CStr(CLng(Me.CustomerDLRLockVersion.Value) + 1)
                End If
                '2013/07/11 TMEJ 小澤 【A.STEP2】次世代e-CRBサービスタブレット 新DB適応に向けた機能開発START
...
#Region "顧客メモ遷移処理"
    ...
    ' ''' 
    ' ''' <history>
    ' ''' 2012/03/09 KN 瀧 【SERVICE_1】顧客メモポップアップを起動を修正
    ' ''' </history>
    ...
    '    Try
    '        'スクリプトで顧客メモポップアップを起動
    '        '2012/03/09 KN 瀧 【SERVICE_1】顧客メモポップアップを起動を修正 START
    ...
    '        JavaScriptUtility.RegisterStartupFunctionCallScript(CType(Me.Page, BasePage), "commitCompleteOpenCustomerMemoEdit", "after")
    '        '2012/03/09 KN 瀧 【SERVICE_1】顧客メモポップアップを起動を修正 END
    '    Finally
    '        ''終了ログの出力
    '        Logger.Info(String.Format(CultureInfo.CurrentCulture _
    '            , "{0}.{1} OUT" _
