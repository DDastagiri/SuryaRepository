...
//更新：2012/06/21 KN 西田 STEP1 重要課題対応
//更新：2012/11/05 TMEJ 彭 問連修正(GMTC121029047)
...
//更新：2013/08/08 TMEJ 成澤 【A.STEP2】タブレット版SMB開発に向けた要件定義
...
//ストール利用ステータスが一部の作業中断中(04)の場合
var C_STALL_USE_STATUS_SOME_INTERRUPTION = "04";
...
var C_ICON_FLAG_P = "1"  //Pアイコンフラグ
var C_ICON_FLAG_L = "2"  //Lアイコンフラグ
var C_ICON_FLAG_0 = "0"    //アイコンの初期化
...
function ReserveChip(aChipId, aStallStartTime, aStallEndTime) {
    ...
    * @param {String}
    */
    this.chipImpVclFlg = C_ICON_FLAG_0;
    //2018/07/06 NSK 可児　TKM Next Gen e-CRB Project Application development Block B-1  Pマーク、L マークなどを表示 END
    /**
...
ReserveChip.prototype = {
    ...
    */
    setChipParameter: function setChipParameter(aDataSet) {
        ...
        //2018/07/06 NSK 可児　TKM Next Gen e-CRB Project Application development Block B-1  Pマーク、L マークなどを表示 START
        this.setImpVclFlg(aDataSet.IMP_VCL_FLG); //imp vcl フラグ
        //2018/07/06 NSK 可児　TKM Next Gen e-CRB Project Application development Block B-1  Pマーク、L マークなどを表示 END
        ...
        //2014/08/29 TMEJ 成澤 【開発】IT9737_NextSTEPサービス ロケ管理の効率化に向けた評価用アプリ作成　START
        this.setParkingCode(aDataSet.PARKINGCODE)
        //2014/08/29 TMEJ 成澤 【開発】IT9737_NextSTEPサービス ロケ管理の効率化に向けた評価用アプリ作成　END
    },
    ...
    createWorkChip: function createWorkChip() {
        ...
        objChipLowerArea.append(this.createPLMarkFlag());
    ...
    createUpperArea: function createUpperArea(objChipUpperArea) {
        ...
        */
        ...
        //2014/08/29 TMEJ 成澤 【開発】IT9737_NextSTEPサービス ロケ管理の効率化に向けた評価用アプリ作成　START
        //実績ステータスが作業待ちの場合
        if (this.chipResultStatus == C_RESULT_STATUS_WAIT && this.parkingCode != " ") {
            //ロケーション番号の表示
            var objChipUpper03 = $("<div />").addClass("titleLine03");
            objChipUpper03.text(this.parkingCode);
            objChipUpperArea.append(objChipUpper03);
            objChipUpper02.css("white-space", "nowrap");
        }
        //2014/08/29 TMEJ 成澤 【開発】IT9737_NextSTEPサービス ロケ管理の効率化に向けた評価用アプリ作成　END
    },
    ...
    //2018/07/04 NSK 可児 TKM Next Gen e-CRB Project Application development Block B-1  Pマーク、L マークなどを表示 START
    createPLMarkFlag: function createPLMarkFlag() {
        var objChipAddIconPL;
        //フラグが1の場合
        if (this.chipImpVclFlg == C_ICON_FLAG_P) {
            //Pマークを追加
            objChipAddIconPL = $("<div>").addClass("IconP");
            //フラグが2の場合
        } else if (this.chipImpVclFlg == C_ICON_FLAG_L) {
            //Lマークを追加
            objChipAddIconPL = $("<div>").addClass("IconL");
        }
        return objChipAddIconPL;
    },
    //2018/07/04 NSK 可児 TKM Next Gen e-CRB Project Application development Block B-1  Pマーク、L マークなどを表示 END
    ...
    createLowerArea03: function createLowerArea03() {
        ...
        else if (this.distance != "" || this.distanceUnit != "") {
    ...
    createChipContents: function createChipContents() {
        ...
        //その他・休憩・使用不可以外のチップを生成する
            ...
            this.createWorkChip();
            //2013/08/08 TMEJ 成澤 【A.STEP2】タブレット版SMB開発に向けた要件定義 START
            //作業中チップの表示順序変更
            if (this.chipResultStatus == C_RESULT_STATUS_WORKING) {
            ...
            //2013/08/08 TMEJ 成澤 【A.STEP2】タブレット版SMB開発に向けた要件定義 END

            //2014/07/23 TMEJ 成澤 【開発】IT9711_タブレットSMB Job Dispatch機能開発 START
            if (this.stallUseStatus == C_STALL_USE_STATUS_SOME_INTERRUPTION) {

                this.objChipsBase.css("background-color", "#FF9999");
            }
            //2014/07/23 TMEJ 成澤 【開発】IT9711_タブレットSMB Job Dispatch機能開発 END
        }
    ...
    */
    setRezEndTime: function setRezEndTime(aRezEndTime) {
        try {
            if (aRezEndTime) {
                //this.chipResultEndTime = new Date(exchangeTimeString(aResultEndTime));
    ...
    */
    ...
    /**
    * imp vclフラグ
    * @param {String} aImpVclFlg
    */
    setImpVclFlg: function setImpVclFlg(aImpVclFlg) {
        if (aImpVclFlg) {
            this.chipImpVclFlg = aImpVclFlg;
        }
    },
    //2018/07/06 NSK 可児　TKM Next Gen e-CRB Project Application development Block B-1  Pマーク、L マークなどを表示 END
    /**
    ...
    */
    setParkingCode: function setParkingCode(aParkingCode) {
        try {
            if (aParkingCode) {
                this.parkingCode = aParkingCode;
            }
        }
        catch (e) {
            this.parkingCode = 0;
