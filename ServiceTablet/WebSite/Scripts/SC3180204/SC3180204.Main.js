...
//    $(".STC04BlockLeftAdviceMemo").fingerScroll();
//    $(".AdviceMemo").fingerScroll();

    //クルクル（アクティビティインジケータ）を表示
    ActiveDisplayOn();
    //再描画（リフレッシュ）タイマーをセットする。
    //※タイマーをリセットするまでの間にscriptエラーなどで動作が止まった際にリフレッシュさせたいため
    ...
    */
    if (null != document.getElementById('ScrollBlock_Engine')) {
        if (383 >= $('#ScrollBlock_Engine').innerHeight()) {
            document.getElementById('ScrollBlock_Engine').style.height = 384 + "px";
        } else {
    ...
    if (null != document.getElementById('ScrollBlock_Inroom')) {
        if (383 >= $('#ScrollBlock_Inroom').innerHeight()) {
            document.getElementById('ScrollBlock_Inroom').style.height = 384 + "px";
        } else {
    ...
    if (null != document.getElementById('ScrollBlock_Left')) {
        if (383 >= $('#ScrollBlock_Left').innerHeight()) {
            document.getElementById('ScrollBlock_Left').style.height = 384 + "px";
        } else {
    ...
    if (null != document.getElementById('ScrollBlock_Right')) {
        if (383 >= $('#ScrollBlock_Right').innerHeight()) {
            document.getElementById('ScrollBlock_Right').style.height = 384 + "px";
        } else {
    ...
    if (null != document.getElementById('ScrollBlock_Under')) {
        if (383 >= $('#ScrollBlock_Under').innerHeight()) {
            document.getElementById('ScrollBlock_Under').style.height = 384 + "px";
        } else {
    ...
    if (null != document.getElementById('ScrollBlock_Trunk')) {
        if (383 >= $('#ScrollBlock_Trunk').innerHeight()) {
            document.getElementById('ScrollBlock_Trunk').style.height = 384 + "px";
        } else {
    ...
    if (null != document.getElementById('ScrollBlock_Maintenance')) {
        if (383 >= $('#ScrollBlock_Maintenance').innerHeight()) {
            document.getElementById('ScrollBlock_Maintenance').style.height = 384 + "px";
        } else {
            document.getElementById('ScrollBlock_Maintenance').style.height = $('#ScrollBlock_Maintenance').innerHeight() + "px";
        }
    }

    // [Register]ボタン押下処理(ViewModeがEdit、且つ、エラーが発生していない、且つ、チップ単位で全Jobが開始している場合、Click処理を実行する)
    $("#ButtonRegisterWork").bind("click", function () {
        ...
        //2019/4/19 [PUAT4226 アドバイスコメント上限対応]対応　End
        ...
        else if (document.getElementById('ViewMode').value != "1" && 
            "" == document.getElementById('ErrorFlg').value &&
            true == document.getElementById('IsAllJobStart').value) {
            ...
            if (true == allcheck()) {
                ...
                HiddenButtonRegisterWork.click();
            } else {
                // 項目チェックNG(エラーメッセージ表示)
    ...
    // [Send]ボタン押下処理(ViewModeがEdit、且つ、エラーが発生していない、且つ、チップ単位で全Jobが開始している場合、Click処理を実行する)
    $("#ButtonSendWork").bind("click", function () {
    ...
    // [Save]ボタン押下処理(ViewModeがEdit、且つ、エラーが発生していない場合、Click処理を実行する)
    $("#ButtonSaveWork").bind("click", function () {
        //2019/4/19 [PUAT4226 アドバイスコメント上限対応]対応　Start
        var adv = document.getElementById("InputStrMsg").value;
        adv = adv.replace(/<br>/g, "\r\n");
        adv = adv.replace(/&lt;/g, "<");
        ...
        //2019/4/19 [PUAT4226 アドバイスコメント上限対応]対応　End
            ...
            HiddenButtonSaveWork.click();
        }
    });

    // [Clear]ボタン押下処理(ViewModeがEdit、且つ、エラーが発生していない場合、Click処理を実行する)
    $("#ButtonClearWork").bind("click", function () {
        if (document.getElementById('ViewMode').value != "1" && "" == document.getElementById('ErrorFlg').value) {
            //全体クルクル表示
            ActiveDisplayOn();

            HiddenButtonClearWork.click();
        }
    });

    // [AdditionalWork]ボタン押下処理(ViewModeがEdit、且つ、エラーが発生していない場合、Click処理を実行する)
    $("#ButtonAdditionalJobWork").bind("click", function () {
        ...
        //2019/4/19 [PUAT4226 アドバイスコメント上限対応]対応　End
            ...
            HiddenButtonAdditionalJobWork.click();
        }
    });

    // ボタン表示切替(遷移元別、検査必要フラグ別)
    if ("1" == document.getElementById('FromTCMain').value) {
        // TCMainからの遷移
        // 検査必要フラグからのRegister/Sendの切り替え
        if ("1" == document.getElementById('SendOrRegister').value) {
            // 要検査:Send
            $("#ButtonSendWork").css("display", "table-cell");
            //$("#ButtonSendWork").css("display", "inline-block");
        } else {
            // 検査不要:Regist
            $("#ButtonRegisterWork").css("display", "table-cell");
            //$("#ButtonRegisterWork").css("display", "inline-block");
        }
        $("#ButtonAdditionalJobWork").css("display", "table-cell");
        //$("#ButtonAdditionalJobWork").css("display", "inline-block");
        $("#ButtonSaveWork").css("display", "table-cell");
        //$("#ButtonSaveWork").css("display", "inline-block");
    } else {
        // TCMain以外からの遷移
        // 検査必要フラグからのRegister/Sendの切り替え
        if ("1" == document.getElementById('SendOrRegister').value) {
        ...
        $("#ButtonClearWork").css("display", "table-cell");
        //$("#ButtonClearWork").css("display", "inline-block");
    ...
    if ((document.getElementById('ViewMode').value != "" && document.getElementById('ViewMode').value != "0") || "" != document.getElementById('ErrorFlg').value) {
        $("#ButtonSendWork").css("background", "#999999");
        $("#ButtonRegisterWork").css("background", "#999999");
        $("#ButtonAdditionalJobWork").css("background", "#999999");
        $("#ButtonSaveWork").css("background", "#999999");
        $("#ButtonClearWork").css("background", "#999999");
    }

    //2014/12/10 [JobDispatch完成検査入力制御開発]対応　Start　↓↓↓

    // チップ単位で開始していないJobが存在した場合、[Send／Register]ボタンを非活性に設定する(※Saveは可能)
    if (false == document.getElementById('IsAllJobStart').value) {
    ...
    /* Swipe */
    /* 2019/10/16　整備名称最大桁数が43文字前後で表示がぎりぎりになってしまう改修　START
    ツールチップと競合するため不要なSWIPE処理を削除
    swipeSetting();
    2019/10/16　整備名称最大桁数が43文字前後で表示がぎりぎりになってしまう改修　END */
...
//    inichangetext();
    ...
    //クルクル（アクティビティインジケータ）を非表示にする
    ActiveDisplayOff();
    //再描画（リフレッシュ）タイマーをリセットする
    commonClearTimer();

});

//20140610 仕様変更対応 Start
...
//        valueChanged: function (num) { $(this).val(num); },
        valueChanged: function (num) {
            valueCnv_OnBlur(this, num, defaultText);

            // 【***完成検査_排他制御***】 start
            posIdxStr = controlName.replace("#" + defaultID, "");
            items = posIdxStr.split("_");

            hiddenDataobj = document.getElementById('HiddenAllData' + items[0] + '_' + items[1]);
            hiddenDataList = hiddenDataobj.value.split("|");

            // 変更を加えた項目の変更フラグを立てる
            hiddenDataList[hiddenDataNo.EditFlag] = 1;
            hiddenDataobj.value = hiddenDataList.join("|");
            $('#EditFlag' + posIdxStr).val(1);
            // 【***完成検査_排他制御***】 end
...
function SelectAccount(gSelectAccount) {
    if ("" != gSelectAccount) {
        document.getElementById("SendUser").value = gSelectAccount.toString();

        //全体クルクル表示
        ActiveDisplayOn();

        HiddenButtonSendWork.click();
    }
}
...
//    var swipeTarget = $("#mainblockContent");
    var swipeTarget = $("#mainblockRegistdata");
...
2019/10/16　整備名称最大桁数が43文字前後で表示がぎりぎりになってしまう改修　END */
// 【***完成検査_排他制御***】 start
function InspecItemsSelectorChange(pos, idx) {
...
function MeainteRadioChange(pos, idx) {
