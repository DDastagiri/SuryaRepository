//------------------------------------------------------------------------------
//SC3220101.MainMenuFingerscroll.js
//------------------------------------------------------------------------------
//機能：メインメニュー（SAステータスマネジメント）[アイコン一覧スクロール用]
//補足：
//作成：2012/05/30 日比野
//------------------------------------------------------------------------------
    ...
    var fn = {
        ...
        */
        constants: {
            ...
            minScrollHeight: 13,
            //フリックリリースの高さ
            flickReleaseHeight: 40,
            //スクロールアニメーションの時間
            animateTimeNormal: 800,
            animateTimeLong: 1600,
            scrollClickControllFlg: "0",
            scrollClickToTagetId: ""
        },
        ...
        */
        init: function (param) {
            ...
            data.scrollMode = "overflow";
            if (param !== undefined && param.scrollMode !== undefined && param.scrollMode) data.scrollMode = param.scrollMode;
        ...
        */
        start: function (e) {
            ...
            if (!event.touches === undefined && event.touches.length > 1) return;

            if (e.data.target.hasClass("fingerscroll-stop") === true) return;

            //内部サイズを計測
            fn.setSize(e.data);
            if (e.data.innerSize.width <= 0 && e.data.innerSize.height <= 0 && e.data.scrollMode === "overflow") {
            ...
            //イベント生成
            $(e.data.target).trigger("start.fingerscroll", fn.getTranslate(e.data));
...
//                    return false;
        ...
        */
        calcScroll: function (data, move, action) {
            ...
            if (action === "drag") {
                var maxScrollHeight = Math.max(data.target.height() * 0.6, 50);
                //ドラッグ処理
                if (top > 0) {
                    //if (top > data.target.height() * 0.6) top = Math.ceil(data.target.height() * 0.6);
                    if (top > maxScrollHeight) top = Math.ceil(maxScrollHeight);
                    top -= fn.constants.dumper * top;
                }
                if (top < -data.innerSize.height && data.innerSize.height > 0) {
                    //if (Math.abs(top + data.innerSize.height) > data.target.height() * 0.6)
                    //    top = -(data.innerSize.height + Math.ceil(data.target.height() * 0.6));
                    if (Math.abs(top + data.innerSize.height) > maxScrollHeight)
                        top = -(data.innerSize.height + Math.ceil(maxScrollHeight));
                    top -= fn.constants.dumper * (top + data.innerSize.height);
                }
            } else {
                top = Math.min(Math.max(Math.min(0, top), -data.innerSize.height), 0);
            }
        ...
        */
        getEventPosition: function () {
            ...
            if (event.touches !== undefined && event.touches && event.touches.length > 0) {
                //iPad移動中
                return { x: event.touches[0].clientX, y: event.touches[0].clientY };
            } else if (event.changedTouches !== undefined && event.changedTouches) {
                //iPad終了
    ...
    //  scrollMode: 「overflow」内部コンテンツがスクロールエリアからはみ出た場合のみ、スクロールする。（規定値）
    //              「all」常にスクロールする。
    // [サンプルコード]
    ...
    //  5.内部コンテンツがスクロールエリアからはみ出てない場合でもスクロールする。
    //     $(selector).fingerScroll({ scrollMode: "all" });
    $.fn.mainMenuFingerScroll = function (param) {
