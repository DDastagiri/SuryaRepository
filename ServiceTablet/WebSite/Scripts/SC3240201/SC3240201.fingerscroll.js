...
(function (window) {
    ...
    var fn = {
        ...
        */
        setTranslate: function (data, position) {
            //2012/12/19 Myose Mod Start
            //data.inner.css({ "transform": "translate3d(" + position.left + "px, " + position.top + "px, 0px)" });
            //2012/12/19 Myose Mod End
            data.inner.css({ "transform": "translate3d(" + position.left + "px, " + "0px, 0px)" });
        },
        ...
        */
        init: function (param) {
            ...
            //スクロール内部用のDIVを作成
            if ($target.find(".scroll-inner-SC3240201").length == 0) {
                $target.wrapInner('<div class="scroll-inner-SC3240201" style="transform:translate3d(0px,0px,0px);left:0px;top:0px;position:relative;" />');
            }
            var $inner = $target.find(".scroll-inner-SC3240201");
    ...
    $.fn.SC3240201fingerScroll = function (param) {
