...
icropScript.ui = {
    ...
    setNotice: function () {
        try {
            //現在表示しているページのチェック
            if (location.pathname.toUpperCase().indexOf("SC3010101") < 0 && location.pathname.toUpperCase().indexOf("SC3090101") < 0 && location.pathname.toUpperCase().indexOf("LOGIN") < 0) {
                //ログインページでない場合は通知アイコンの更新処理を行う
    ...
    openDialog: function (url, effect, postbackCallBack) {
        ...
        $iframe.bind("load", function (e) {
            ...
            //位置設定
            $wrap.css({ "transform": "translate3d(" + left + "px, " + top + "px, 0px)" });
            $iframe.unbind("load", arguments.callee);
            if (effect === "left") {
                //左からスライドイン
                $wrap.css({ "transform": "translate3d(" + ($wrap.outerWidth() * -1) + "px, " + top + "px, 0px)" });
                left = 0;
            } else if (effect === "right") {
                //右からスライドイン
                $wrap.css({ "transform": "translate3d(" + ($wrap.outerWidth() + $overlay.outerWidth()) + "px, " + top + "px, 0px)" });
                left = $overlay.outerWidth() - $wrap.outerWidth();
            } else if (effect == "top") {
                //上からスライドイン
                $wrap.css({ "transform": "translate3d(" + left + "px, " + ($wrap.outerHeight() * -1) + "px, 0px)" });
                top = 0;
            } else if (effect == "bottom") {
                //下からスライドイン
                $wrap.css({ "transform": "translate3d(" + left + "px, " + ($wrap.outerHeight() + $overlay.outerHeight()) + "px, 0px)" });
    ...
    //三角表示処理
    backupArrow: new Array(),    //三角ボタン表示オブジェクト格納用
    arrowImageOn: function (buttonID) {
        var setArrow = "Arrow".concat(buttonID);
        var newArrow = document.getElementById(setArrow);
        //引数の三角を表示する
        newArrow.style.display = "";
        //表示した参画をオブジェクトに格納する
        icropScript.ui.backupArrow.push(newArrow);
    },
    //三角非表示処理
    arrowImageOff: function () {
        //いままで表示した三角アイコンを消す
        for (i = 0; i < icropScript.ui.backupArrow.length; i++) {
            var newArrow = icropScript.ui.backupArrow[i];
            newArrow.style.display = "none";
        }
        //格納したものを初期化
        icropScript.ui.backupArrow = new Array();
    }
