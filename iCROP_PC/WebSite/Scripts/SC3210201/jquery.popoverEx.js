...
(function ($) {
    $.fn.popoverEx = function (options) {
        ...
        function togglePopover(button, show) {
            ...
            // t.mizumoto その他イベントに影響を与えるため、Clickイベントは無効にしておく
            /*
        ...
        if (settings.contentId != null && settings.contentId.length > 0) {
            ...
            settings.popover$ = $('<div class="popoverEx" id="' + settings.id + '">'
        ...
        $("#InnerMainBlock").unbind(".popoverEx");

        // t.mizumoto その他イベントに影響を与えるため、Clickイベントは無効にしておく
        /*
        // document click closes active popover		
        $("#InnerMainBlock").bind("click.popoverEx", function (event) {
            if (($(event.target).parents(".popoverEx").length == 0)
					&& (!$(event.target).hasClass('popover-button'))) {

                //alert("#InnerMainBlock" + $(event.target).html());
        ...
        */
        ...
        $("#InnerMainBlock").bind("hideOpenPopover.popoverEx", hideOpenPopover);

        // document Esc key listener
        var selector = this;
        if (settings.closeOnEsc) {
            $("#InnerMainBlock").bind('keydown', function (event) {
        ...
        if (!settings.live) {
            return this.each(function () {
                ...
                button.bind('click', function () {
                    ...
                    // t.mizumoto クリックイベントを無効にしておかないとPageMethodが二重で呼び出されてしまうためreturn falseしておく
                    return false;
                });
